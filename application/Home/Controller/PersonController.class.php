...
class PersonController extends Controller{
    ...
    public function delfri(){
        $linkid=$_GET['linkid'];
        $link=M("link")->where("linkid=$linkid")->find();
        $userid=$link['userid'];
        $friend=$link['friendid'];
        M("link")->where("userid=$userid and friendid=$friend")->delete();
        M("link")->where("friendid=$userid and userid=$friend")->delete();
        $this->redirect("Person/index");
    }

    public function delete(){
        $circleid=$_GET['circleid'];
        M("reviews")->where("circleid=$circleid")->delete();
        $rs = M('circleimage')->where("circleid=$circleid")->select();
        foreach($rs as $k => $v){
            unlink($v['picpath']);
        }
        M("circleimage")->where("circleid=$circleid")->delete();
        M("circle")->where("circleid=$circleid")->delete();
    ...
    public function modify($userid){
        $result  = M('user')->where("userid = {$userid} ")->save($_POST);
        if ($result > 0) {
            echo 1;
        }else {
            echo 0;
    ...
    public function wantmore(){
        ...
        foreach($circle as $k=>$v) {
            ...
            <a class="delete" href="'.__APP__.'/Person/delete/circleid/'.$v['circleid'].'" style="width: 60px;height: 30px;border: 1px solid dodgerblue;border-radius: 3px;float: right;margin: 5px;text-align: center;line-height: 30px;text-decoration: none;color: dodgerblue">删除</a>
